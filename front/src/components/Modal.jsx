...
const Modal = () => {
  ...
  const handleChange = (e) => {
    ...
    setFormData((prev) => ({
      ...prev,
      [name]: type === 'checkbox' ? checked : value, //name 입력값을 받는데, 만약 타입이 checkbox이면 cheked 값을 받고 아니면 value 값을 받는다.
    }));
  };
  ...
  return (
    <div className="modal fixed bg-black bg-opacity-50 flex items-center justify-center w-full h-full left-0 top-0 z-50">
      <div className="form-wrapper bg-gray-700 rounded-md w-1/2 flex flex-col items-center relative p-4">
        <h2 className="text-2xl py-2 border-b border-gray-300 w-fit font-semibold font-customFontKR">
          할 일 추가
        </h2>
